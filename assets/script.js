...
var quizQuestions = [
    {
        ...
        options: [":onhover", "element.hover", ":hover", "(hover = id)"],
        correctAnswer: ":hover",
    },
    {
    ...
    },
    {
        ...
        options: ["<script>", "<sylesheet>", "<color>", "<style>"],
        correctAnswer: "<style>",
    },
    {
        question: "How do you store items in your javascript?",
        options: ["Object", "Boolean", "script", "HTML"],
...
function updateTimer() {
    //timer function, timer starts at constant 60 then updates minus 1 each time its called
    timer--;
    ...
    if (timer === 0) {
        //when timer hits 0 the endquiz function and stoptimer function stop.
        stopTimer();
        endQuiz();
    }
}
...
function startTimer() {
    //timer start function calls update timer function every 1000 miliseconds
    timerElement.textContent = timer;
    intervalId = setInterval(updateTimer, 1000);
}

function stopTimer() {
    //stop timer function with clearInterval
...
function checkAnswer(selectedOption) {
    //defines checkAnswer under the selected question
    const question = quizQuestions[currentQuestionIndex]; //the number the index is on is the question

    if (selectedOption === question.correctAnswer) {
        //if user inputed option is equal to the current questions correct answer
        ...
        //or else if the answer was wrong it minus's 10 seconds of the timers current time
        timer -= 10;
